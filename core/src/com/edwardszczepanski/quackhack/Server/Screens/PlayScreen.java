...
public class PlayScreen implements Screen, NetListener {
    ...
    public PlayScreen(QuackHack game) {
        ...
        Double number = Math.random() * 24;
        if(number > 24 - 8){
            gameMusic = Gdx.audio.newMusic(Gdx.files.internal("Carnival.wav"));
        }
        else if(number > 24 - 8 *2){
            gameMusic = Gdx.audio.newMusic(Gdx.files.internal("Jungle.wav"));
        }
        else {
            gameMusic = Gdx.audio.newMusic(Gdx.files.internal("Meadow.wav"));
        }
    ...
    public RayHandler rayHandlerGenerator(){
        ...
        localRay.setAmbientLight(0.82f, 0.82f, 0.82f, 0.3f);
